...
const allEnv = z.object({
  ...
  CRAWLER_MAX_CONCURRENCY: z.coerce.number().default(5),
  CRAWLER_RETRY_LIMIT: z.coerce.number().default(3),
  CRAWLER_RETRY_DELAY: z.coerce.number().default(1000),
  CRAWLER_RATE_LIMIT_WINDOW: z.coerce.number().default(60000),
  CRAWLER_MAX_REQUESTS_PER_WINDOW: z.coerce.number().default(30),
  CRAWLER_USE_PROXY: stringBool("false"),
  CRAWLER_PROXY_LIST: z.string().optional(),
  CRAWLER_RANDOM_DELAY: stringBool("true"),
...
const serverConfigSchema = allEnv.transform((val) => {
  return {
    ...
    inference: {
      ...
      zhipuApiKey: val.ZHIPU_API_KEY,
      zhipuBaseUrl: val.ZHIPU_BASE_URL,
    ...
    crawler: {
      ...
      maxConcurrency: val.CRAWLER_MAX_CONCURRENCY,
      retryLimit: val.CRAWLER_RETRY_LIMIT,
      retryDelay: val.CRAWLER_RETRY_DELAY,
      rateLimitWindow: val.CRAWLER_RATE_LIMIT_WINDOW,
      maxRequestsPerWindow: val.CRAWLER_MAX_REQUESTS_PER_WINDOW,
      useProxy: val.CRAWLER_USE_PROXY,
      proxyList: val.CRAWLER_PROXY_LIST,
      randomDelay: val.CRAWLER_RANDOM_DELAY,
    },
    ocr: {
      langs: val.OCR_LANGS,
